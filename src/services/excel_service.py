...
class ExcelService:
    ...
    def generate_dfs_from_response_data(self, response_data: dict) -> list[pd.DataFrame | None]:
        list_dfs = []
        for _, title, _, statistic in response_data.get('data'):
            df = pd.DataFrame(data=statistic, columns=['Время', f'Вход_{title}', f'Выход_{title}'])
            list_dfs.append(df)
        
        linked_df = reduce(lambda left, right: pd.merge(left, right, on='Время'), list_dfs)
        return linked_df
